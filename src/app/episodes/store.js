...
export const fetchShowEpisodeList = createAsyncThunk(
	`${PREFIX}/fetchShowEpisodeList`,
	async (showId, thunkAPI) => {
		const result = await thunkAPI.extra.showsService.getShowEpisodeList(
			showId
		)
		return result.data
	}
...
export const fetchEpisodeMainInformation = createAsyncThunk(
	`${PREFIX}/fetchEpisodeMainInformation`,
	async ({ showId, episodeId }, thunkAPI) => {
		const episode = episodesSelectors.selectById(
			thunkAPI.getState(),
			episodeId
		)
		...
		const result = await thunkAPI.extra.episodesService.getEpisodeMainInformation(
			episodeId
		)
		return result.data
	}
